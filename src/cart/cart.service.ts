...
export class CartService {
  ...
  public getCartWithCartDetail = async(uuid: string) => {
    ...
    const cart = await this.cartRepository.getCartByUserId(users[0]._id) as unknown as Cart;
    if (cart === null) return [];

    const cartDetails = await this.getCartDetailByCartId(cart._id);
  ...
  private getCartDetailByCartId = async(cartId: string) => {
    const cartDetails = (await this.cartDetailRepository.getCartDetailByCartId(cartId)) as unknown as CartDetail[];
  ...
  public getCart = async (uuid:string) => {
    ...
    return await this.cartRepository.getCartByUserId(users[0]._id) as unknown as Cart;
  }
